    ...
    struct nfc_platform_data *data)
{
	...
	data->clk_req_gpio = of_get_named_gpio(np, "nxp,pn544-clk-req", 0);
	if ((!gpio_is_valid(data->clk_req_gpio)))
		data->clk_req_gpio =  -EINVAL;

	pr_info("%s: %d, %d, %d, %d,%d error:%d\n", __func__,
			data->irq_gpio, data->ven_gpio, data->firm_gpio,
			data->ese_pwr_gpio, data->clk_req_gpio, ret);
	return ret;
}
        ...
        const struct i2c_device_id *id)
{
	...
	if (gpio_is_valid(platform_data.clk_req_gpio)) {
		ret = gpio_request(platform_data.clk_req_gpio, "nfc-clk_req");
		if (ret) {
			pr_err("%s: unable to request nfc clk req gpio [%d]\n", __func__, platform_data.clk_req_gpio);
			goto err_firm_gpio;
		}
		ret = gpio_direction_input(platform_data.clk_req_gpio);
		if (ret) {
			pr_err("%s: cannot set direction for nfc clk req gpio [%d]\n",
					__func__, platform_data.clk_req_gpio);
			goto err_ese_pwr_gpio;
		}
	} else {
		/* ese gpio not required for latest platform*/
		pr_info("%s: clk req gpio not provided\n", __func__);
	}
	...
	nfc_dev->clk_req_gpio = platform_data.clk_req_gpio;
...
static void check_hw_info(struct nfc_dev *nfc_dev) {
	...
	if (ret > 0) {
		...
		case NFCC_NQ_220:
			...
			break;
        case NFCC_NQ_310:
			if (hw_info.data[4] == 0x12)
				push_component_info(NFC, "PN557", "NXP");
			else
				push_component_info(NFC, "PN553", "NXP");
            break;
        case NFCC_NQ_310_V2:
			if (hw_info.data[4] == 0x12)
				push_component_info(NFC, "PN557", "NXP");
