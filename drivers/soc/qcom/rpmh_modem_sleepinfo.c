...
#define MODEM_SLEEPINFO_BUFFER_SIZE (1024)
...
static unsigned int proc_modemsleep_perms = 0444;
...
static const char restart_string[] = "ONEPLUS_MARK_RESTART";
		...
		const char __user *buffer, size_t count, loff_t *ppos)
{
	...
	if (count < modeminfo_stats.buf_size - modeminfo_stats.wr_offset)
		cp_count = count;
	else
		cp_count = modeminfo_stats.buf_size - modeminfo_stats.wr_offset;
	if (cp_count < 0)
		cp_count = 0;
...
static int __init rpmh_modem_sleepinfo_init(void)
{
	...
	modeminfo_stats.buf_base = kzalloc(MODEM_SLEEPINFO_BUFFER_SIZE, GFP_KERNEL);
